...
using style_type = std::underlying_type_t<style>;

/// A list of all icon-names with the codepoint (unicode-value) on the right
/// You can use the names on the page
...
/// This class requires a 2-phase construction. You must first create the class and then
/// initialize it via an init* method
class QtAwesome : public QObject {
 ...
 public:
  ...
  const QHash<QString, fa> namedCodePoints(style st) const;

  void     setDefaultOption(const QString& name, const QVariant& value);
  QVariant defaultOption(const QString& name);

  QIcon icon(style st, fa character, const QVariantMap& options = QVariantMap());
  ...
  QFont font(style st, int size) const;

  /// Returns the font-name that is used as icon-map
  QString fontName(style st) const;

 private:
  style         stringToStyleEnum(const QString st) const;
  const QString styleEnumToString(style st) const;
 ...
 private:
  ...
  QHash<style_type, FontData> _fontDetails;  ///< The fonts name used for each style
  QHash<style_type, QHash<QString, fa>*>
      _namedCodepoints;  ///< A map with names mapped to code-points for each style
