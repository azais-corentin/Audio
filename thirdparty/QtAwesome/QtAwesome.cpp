...
class QtAwesomeCharIconPainter : public QtAwesomeIconPainter {
 protected:
  ...
  QStringList optionKeysForModeAndState(const QString& key,
                                        QIcon::Mode    mode,
                                        QIcon::State   state)
  {
    QString modePostfix;
  ...
  QVariant optionValueForModeAndState(const QString&     baseKey,
                                      QIcon::Mode        mode,
                                      QIcon::State       state,
                                      const QVariantMap& options)
  {
    foreach (QString key, optionKeysForModeAndState(baseKey, mode, state)) {
      // if ( options.contains(key) && options.value(key).toString().isEmpty()) qDebug()
      // << "Not found:" << key;
 ...
 public:
  virtual void paint(QtAwesome*         awesome,
                     QPainter*          painter,
                     const QRect&       rect,
                     QIcon::Mode        mode,
                     QIcon::State       state,
                     const QVariantMap& options)
  {
    ...
    QColor color =
        optionValueForModeAndState("color", mode, state, options).value<QColor>();
    QString text = optionValueForModeAndState("text", mode, state, options).toString();
    auto    st   = static_cast<style>(
        optionValueForModeAndState("style", mode, state, options).toInt());
    ...
    if (tbr.width() > textRect.width()) {
      drawSize = static_cast<int>(ft.pixelSize() *
                                  qMin(textRect.width() * 0.95 / tbr.width(),
                                       textRect.height() * 0.95 / tbr.height()));
    ...
    if (st == style::duotone) {
      QColor dcolor = optionValueForModeAndState("duotone-color", mode, state, options)
                          .value<QColor>();
...
class QtAwesomeIconPainterIconEngine : public QIconEngine {
 public:
  QtAwesomeIconPainterIconEngine(QtAwesome*            awesome,
...
QtAwesome::QtAwesome(QObject* parent) : QObject(parent), _namedCodepoints()
{
  ...
  _fontDetails.insert(static_cast<style_type>(style::solid),
                      FontData(FAS_FONT_FILENAME, FAS_FONT_WEIGHT));
  _fontDetails.insert(static_cast<style_type>(style::regular),
                      FontData(FAR_FONT_FILENAME, FAR_FONT_WEIGHT));
  _fontDetails.insert(static_cast<style_type>(style::brands),
                      FontData(FAB_FONT_FILENAME, FAB_FONT_WEIGHT));
#ifdef FONT_AWESOME_PRO
  _fontDetails.insert(static_cast<style_type>(style::light),
                      FontData(FAL_FONT_FILENAME, FAL_FONT_WEIGHT));
  _fontDetails.insert(static_cast<style_type>(style::duotone),
                      FontData(FAD_FONT_FILENAME, FAD_FONT_WEIGHT));
#endif
}
...
QtAwesome::~QtAwesome()
{
  ...
  if (_namedCodepoints.contains(static_cast<style_type>(style::brands)))
    delete _namedCodepoints[static_cast<style_type>(style::brands)];
#ifdef FONT_AWESOME_PRO
  if (_namedCodepoints.contains(static_cast<style_type>(style::solid)))
    delete _namedCodepoints[static_cast<style_type>(style::solid)];
#else
...
/// this method return true on success, it will return false if the fnot cannot be
/// initialized To initialize QtAwesome with font-awesome you need to call this method
bool QtAwesome::initFontAwesome()
{
  bool errors = false;
  // The macro below internally calls "qInitResources_QtAwesome()". this initializes
  // the resource system. For a .pri project this isn't required, but when building and
  // using a static library the resource need to initialized first.
  ///
  // I've checked th qInitResource_* code and calling this method mutliple times shouldn't
  // be any problem (More info about this subject: http://qt-project.org/wiki/QtResources)
#ifdef FONT_AWESOME_PRO
  ...
  auto brands = new QHash<QString, fa>();
  ...
  _namedCodepoints.insert(static_cast<style_type>(style::brands), brands);

  // initialize others code icons maps
#ifdef FONT_AWESOME_PRO
  auto commonMap = new QHash<QString, fa>();
  ...
  _namedCodepoints.insert(static_cast<style_type>(style::regular), commonMap);
  _namedCodepoints.insert(static_cast<style_type>(style::solid), commonMap);
  _namedCodepoints.insert(static_cast<style_type>(style::light), commonMap);
  _namedCodepoints.insert(static_cast<style_type>(style::duotone), commonMap);
#else
  auto farMap = new QHash<QString, fa>();
  for (unsigned i = 0;
       i < sizeof(faCommonIconArray) / sizeof(FANameIcon) && i < FREE_REGULAR_ICON_SIZE;
       ++i) {
...
const QHash<QString, fa> QtAwesome::namedCodePoints(style st) const
{
  auto it = _namedCodepoints.find(static_cast<style_type>(st));
  if (it == _namedCodepoints.end())
    return QHash<QString, fa>();
...
/// </code>
QIcon QtAwesome::icon(style st, fa character, const QVariantMap& options)
{
  ...
  optionMap.insert("style", static_cast<int>(st));
...
/// https://fontawesome.com/cheatsheet/pro adding the style prefix, e.g. "fas
/// address-book"
...
QIcon QtAwesome::icon(const QString& name, const QVariantMap& options)
{
  ...
  if (_namedCodepoints.contains(static_cast<style_type>(st)) &&
      _namedCodepoints[static_cast<style_type>(st)]->contains(ic)) {
    return icon(st, _namedCodepoints[static_cast<style_type>(st)]->value(ic), options);
  }

  // create a merged QVariantMap to have default options and icon-specific options
  QVariantMap optionMap = mergeOptions(_defaultOptions, options);
  optionMap.insert("style", static_cast<style_type>(st));
...
QIcon QtAwesome::icon(QtAwesomeIconPainter* painter, const QVariantMap& optionMap)
{
  ...
  // QIcon's placed in gui items are often cached and not deleted when my memory-leak
  // detection checks for leaks. I'm not sure if it's a Qt bug or something I do wrong
  QtAwesomeIconPainterIconEngine* engine =
      new QtAwesomeIconPainterIconEngine(this, painter, optionMap);
  return QIcon(engine);
}
...
/// \brief QtAwesome::font Creates/Gets the icon font with a given size in pixels. This
/// can be usefull to use a label for displaying icons
///
...
QFont QtAwesome::font(style st, int size) const
{
  auto it = _fontDetails.find(static_cast<style_type>(st));
  if (it == _fontDetails.end())
    return {};

  QFont ft(it->fontFamily());
  ft.setPixelSize(size);
  ft.setWeight(it->fontWeight());

  return ft;
}

QString QtAwesome::fontName(style st) const
{
  auto it = _fontDetails.find(static_cast<style_type>(st));
  if (it != _fontDetails.end())
    return it->fontFamily();

  return "";
}

style QtAwesome::stringToStyleEnum(const QString st) const
{
  if (st == "fas")
    return style::solid;
  else if (st == "far")
...
const QString QtAwesome::styleEnumToString(style st) const
{
  switch (st) {
    case style::brands:
      return "fab";
