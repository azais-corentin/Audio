...
std::vector<fft::fcomplex> fft::r2c(std::vector<float>& input)
{
  ...
  auto p = fftwf_plan_dft_r2c_1d(static_cast<int>(nInput), input.data(),
...
std::vector<float> fft::c2r(std::vector<fft::fcomplex>& input)
{
  ...
  output.reserve(nOutput);
  output.resize(nOutput);
  auto p = fftwf_plan_dft_c2r_1d(static_cast<int>(nInput),
...
std::vector<fft::fcomplex> fft::c2c(std::vector<fft::fcomplex>& input, Direction d)
{
  ...
  auto p = fftwf_plan_dft_1d(static_cast<int>(nInput),
                             reinterpret_cast<fftwf_complex*>(input.data()),
                             reinterpret_cast<fftwf_complex*>(output.data()),
                             static_cast<int>(d), FFTW_ESTIMATE | FFTW_PRESERVE_INPUT);
