...
template <class Derived> class BaseInterface {
  public:
    ...
    inline float time(std::size_t sample) const { return static_cast<float>(sample) / m_sample_rate; }

  private:
    friend Derived;
    BaseInterface() = default;
    BaseInterface(std::size_t length, std::size_t sample_rate) : m_length(length), m_sample_rate(sample_rate) {}
