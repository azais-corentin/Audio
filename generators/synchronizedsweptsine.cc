...
SynchronizedSweptSine::SynchronizedSweptSine(float       f1,
                                             float       f2,
                                             std::size_t length,
                                             std::size_t sample_rate)
    : BaseInterface<SynchronizedSweptSine>(length, sample_rate),
      m_f1(f1),
      m_f2(f2),
...
{
  if (!finished(index))
    return std::sin(2.f * std::numbers::pi_v<float> * m_f1 * m_L *
                    std::exp(time(index) / m_L));
...
std::vector<std::complex<float>> SynchronizedSweptSine::xtilde() const
{
  ...
  std::generate(output.begin(), output.end(), [&, k = 0]() mutable {
    ...
    return 2.f * std::sqrt(f / m_L) *
           std::exp(-2if * std::numbers::pi_v<float> * f * m_L *
                        (1 - std::log(f / m_f1)) +
